...
const summaryCards = [
	{ label: "Total balance", value: "$15,700.00", delta: "+12.1% vs last month", positive: true },
	{ label: "Income", value: "$8,500.00", delta: "+6.3% vs last month", positive: true },
	{ label: "Expense", value: "$6,222.00", delta: "-2.4% vs last month", positive: false },
	{ label: "Total savings", value: "$32,913.00", delta: "+12.1% vs last month", positive: true },
];
...
const transactions = [
	{ date: "25 Jul 12:30", amount: "$10", name: "YouTube", method: "VISA *3254", category: "Subscription", color: "#7c8cf1" },
	{ date: "26 Jul 15:00", amount: "$150", name: "Reserved", method: "Mastercard *2154", category: "Shopping", color: "#f18fb5" },
	{ date: "27 Jul 9:00", amount: "$80", name: "Yaposhka", method: "Mastercard *2154", category: "Cafe & Restaurants", color: "#7c8cf1" },
];
			...
			<section className={styles.summaryGrid}>
				{summaryCards.map((card) => (
					<article key={card.label} className={styles.summaryCard}>
						...
						<div className={card.positive ? styles.deltaPositive : styles.deltaNegative}>
							{card.delta}
						</div>
					</article>
				))}
					...
					<div className={styles.table}>
						...
						{transactions.map((tx) => (
							<div key={tx.date + tx.name} className={styles.tableRow}>
								...
								<span>{tx.name}</span>
								<span>{tx.method}</span>
								<span className={styles.chip} style={{ background: `${tx.color}22`, color: tx.color }}>
									{tx.category}
								</span>
							</div>
						))}
